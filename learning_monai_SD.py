...
class classifier_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        image = np.load(self.image_files[index]).astype(np.float32)
...
if __name__ == "__main__":
    ...
    print(root_dir)
    training_dir = "/cs/student/project_msc/2025/aibh/jgomezbe/UCSD-PTGBM/MONAI_data/train"
    val_dir = "/cs/student/project_msc/2025/aibh/jgomezbe/UCSD-PTGBM/MONAI_data/val"
    test_dir = "/cs/student/project_msc/2025/aibh/jgomezbe/UCSD-PTGBM/MONAI_data/test"
    ...
    image_width, image_height = np.load(train_x[0]).shape
    ...
    val_transforms = Compose([EnsureChannelFirst(channel_dim="no_channel"), ScaleIntensity()])
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                if result > best_metric:
                    ...
                    torch.save(model.state_dict(), os.path.join(root_dir, "best_metric_model_SD.pth"))
    ...
    model.load_state_dict(torch.load(os.path.join(root_dir, "best_metric_model_SD.pth"), weights_only=True))
    model.eval()
    y_true = []
    y_pred = []
    with torch.no_grad():
