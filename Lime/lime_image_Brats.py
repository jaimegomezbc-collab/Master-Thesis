...
class LimeImageExplainer(object):
    ...
    def explain_instance(self, image, classifier_fn, labels=(1,),
        ...
        Args:
            ...
            n_modes: Number of input modes (1 for single image, 3 for three-input multimodal)
                ...
                if hide_color is None:
                    for x in np.unique(segments):
                        fudged_image[segments == x] = (
                            np.mean(current_image[segments == x][:, 0]),
                            np.mean(current_image[segments == x][:, 1]),
    ...
    def data_labels(self,
        ...
        for idx in rows:
            ...
            if len(imgs) == batch_size:
                if n_modes != 1:
                    preds = classifier_fn(np.array(imgs), n_modes=n_modes)
                else:
